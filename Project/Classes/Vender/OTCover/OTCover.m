...
- (OTCover*)initWithTableViewWithHeaderImage:(UIImage*)headerImage withOTCoverHeight:(CGFloat)height{
    ...
    self = [[OTCover alloc] initWithFrame:CGRectMake(0, 0, bounds.size.width, bounds.size.height)];
...
- (void)handleTap
{
    if ([self.delegate respondsToSelector:@selector(hcotcoreSelectedImageView)])
    {
        [self.delegate hcotcoreSelectedImageView];
    }
}
