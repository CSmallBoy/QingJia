...
@interface HCMTalkAfterSaleCell ()
...
@implementation HCMTalkAfterSaleCell

+(instancetype)cellWithTable:(UITableView *)tableView
{
    static NSString *ID = @"M-TalkMyOrderCell";
    
    HCMTalkAfterSaleCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell =[[HCMTalkAfterSaleCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
        [cell addSubviews];
    }
...
-(void)setInfo:(HCMtalkMyOrderInfo *)info
{
    ...
    if (info.doWhat.length > 0)
    {
        [self.chooseBtn setTitle:@"取消退货" forState:UIControlStateNormal];
        [self.againBtn setTitle:@"售后跟踪" forState:UIControlStateNormal];
    }
    else
    {
        [self.chooseBtn removeFromSuperview];
        [self.againBtn setTitle:@"申请售后" forState:UIControlStateNormal];
    }
